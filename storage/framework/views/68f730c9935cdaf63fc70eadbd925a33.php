    ...
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-3 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400"><?php echo e($logs->total()); ?> records</p>
            </div>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5 capitalize">
                                    <?php echo e(str_replace('_', ' ', $log->file_type ?? '')); ?>

                                </p>
                                <?php else: ?>
                                
                                <?php if($log->modelRecord): ?>
                                <p class="text-sm text-gray-500 italic"><?php echo e($log->modelRecord->title ?? 'N/A'); ?></p>
                                <?php else: ?>
                                <span class="text-sm text-gray-400 italic">Deleted</span>
                                <?php endif; ?>
                                <?php endif; ?>
                                <?php if($log->modelRecord): ?>
                                <p class="text-xs text-[#0071c5] mt-0.5 hover:underline">
                                    <a href="<?php echo e($log->model === 'campaign'
                ? route('campaigns.show', $log->modelRecord)
                : route('assets.show', $log->modelRecord->slug)); ?>">
                                        <?php echo e($log->modelRecord->title); ?>

                                    </a>
                                </p>
                                <?php else: ?>
                                <p class="text-xs text-gray-400 italic mt-0.5">Deleted</p>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <div class="w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                                        <span class="text-xs font-bold text-green-600 dark:text-green-400"><?php echo e($log->count); ?></span>
                                    </div>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <p class="text-sm text-gray-500 dark:text-gray-400"><?php echo e($log->updated_at->format('M d, Y')); ?></p>
                                <p class="text-xs text-gray-400"><?php echo e($log->updated_at->diffForHumans()); ?></p>
                            </td>
            ...
            <div class="flex items-center justify-between">
                ...
                <button disabled class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-medium text-gray-700 opacity-50 cursor-not-allowed dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M2.58301 9.99868C2.58272 10.1909 2.65588 10.3833 2.80249 10.53L7.79915 15.5301C8.09194 15.8231 8.56682 15.8233 8.85981 15.5305C9.15281 15.2377 9.15297 14.7629 8.86018 14.4699L5.14009 10.7472L16.6675 10.7472C17.0817 10.7472 17.4175 10.4114 17.4175 9.99715C17.4175 9.58294 17.0817 9.24715 16.6675 9.24715L5.14554 9.24715L8.86017 5.53016C9.15297 5.23717 9.15282 4.7623 8.85983 4.4695C8.56684 4.1767 8.09197 4.17685 7.79917 4.46984L2.84167 9.43049C2.68321 9.568 2.58301 9.77087 2.58301 9.99715Z" fill="currentColor" />
                    </svg>
                    Previous
                </button>
                <?php else: ?>
                <a href="<?php echo e($logs->previousPageUrl()); ?>" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M2.58301 9.99868C2.58272 10.1909 2.65588 10.3833 2.80249 10.53L7.79915 15.5301C8.09194 15.8231 8.56682 15.8233 8.85981 15.5305C9.15281 15.2377 9.15297 14.7629 8.86018 14.4699L5.14009 10.7472L16.6675 10.7472C17.0817 10.7472 17.4175 10.4114 17.4175 9.99715C17.4175 9.58294 17.0817 9.24715 16.6675 9.24715L5.14554 9.24715L8.86017 5.53016C9.15297 5.23717 9.15282 4.7623 8.85983 4.4695C8.56684 4.1767 8.09197 4.17685 7.79917 4.46984L2.84167 9.43049C2.68321 9.568 2.58301 9.77087 2.58301 9.99715Z" fill="currentColor" />
                    </svg>
                    Previous
                ...
                <a href="<?php echo e($logs->nextPageUrl()); ?>" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                    Next
                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M17.4175 9.9986C17.4178 10.1909 17.3446 10.3832 17.198 10.53L12.2013 15.5301C11.9085 15.8231 11.4337 15.8233 11.1407 15.5305C10.8477 15.2377 10.8475 14.7629 11.1403 14.4699L14.8604 10.7472L3.33301 10.7472C2.91879 10.7472 2.58301 10.4114 2.58301 9.99715C2.58301 9.58294 2.91879 9.24715 3.33301 9.24715L14.8549 9.24715L11.1403 5.53016C10.8475 5.23717 10.8477 4.7623 11.1407 4.4695C11.4336 4.1767 11.9085 4.17685 12.2013 4.46984L17.1588 9.43049C17.3173 9.568 17.4175 9.77087 17.4175 9.99715Z" fill="currentColor" />
                    </svg>
                </a>
                <?php else: ?>
                <button disabled class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-medium text-gray-700 opacity-50 cursor-not-allowed dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\asset-management\resources\views/download-logs/index.blade.php ENDPATH**/ ?>