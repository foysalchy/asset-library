...
<section class="max-w-screen-2xl mx-auto px-4 lg:px-8 py-10 font-['Outfit']">
    ...
    <div class="flex items-start justify-between mb-8">
        <div class="flex items-center gap-4">
            <a href="<?php echo e(route('tickets.index')); ?>" class="text-[#0071c5] hover:opacity-70 transition-colors">
                <i class="fas fa-arrow-left text-xl"></i>
            </a>
            <div>
                <h1 class="text-2xl font-light text-[#0071c5]"><?php echo e($ticket->subject); ?></h1>
                <p class="text-xs text-gray-400 mt-1">Ticket #<?php echo e($ticket->id); ?> · Opened on <?php echo e($ticket->created_at->format('d M Y')); ?></p>
            </div>
        </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col" style="height:650px;">
                ...
                <div class="flex-1 overflow-y-auto px-4 py-6 space-y-6 bg-gray-50/50" id="chatBox">
                    ...
                    <div class="flex items-end gap-2 <?php echo e($ticket->user_id === auth()->id() ? 'flex-row-reverse' : ''); ?>">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-[10px] font-bold shrink-0 <?php echo e($ticket->user_id === auth()->id() ? 'bg-[#001e3e]' : 'bg-[#0071c5]'); ?>">
                            <?php echo e(strtoupper(substr($ticket->user->name, 0, 1))); ?>

                        </div>
                        <div class="max-w-[75%]">
                            <p class="text-[10px] text-gray-400 mb-1 <?php echo e($ticket->user_id === auth()->id() ? 'text-right mr-1' : 'ml-1'); ?>">
                                <?php echo e($ticket->user->name); ?> · <?php echo e($ticket->created_at->diffForHumans()); ?>

                            </p>
                            <div class="px-4 py-3 shadow-sm <?php echo e($ticket->user_id === auth()->id() ? 'bg-[#0071c5] text-white rounded-tl-2xl rounded-bl-2xl rounded-br-2xl' : 'bg-white border border-gray-200 text-gray-800 rounded-tr-2xl rounded-br-2xl rounded-bl-2xl'); ?>">
                                <p class="text-sm leading-relaxed"><?php echo e($ticket->description); ?></p>
                            </div>
                            <?php if($ticket->image): ?>
                                <img src="<?php echo e($ticket->image_url); ?>" alt="attachment" class="mt-2 max-w-[250px] rounded-xl <?php echo e($ticket->user_id === auth()->id() ? 'ml-auto' : ''); ?> block border border-gray-100">
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <?php $__currentLoopData = $ticket->replies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reply): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-end gap-2 <?php echo e($reply->user_id === auth()->id() ? 'flex-row-reverse' : ''); ?>">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-[10px] font-bold shrink-0 <?php echo e($reply->user_id === auth()->id() ? 'bg-[#001e3e]' : 'bg-[#0071c5]'); ?>">
                                <?php echo e(strtoupper(substr($reply->user->name, 0, 1))); ?>

                            </div>
                            <div class="max-w-[75%]">
                                <p class="text-[10px] text-gray-400 mb-1 <?php echo e($reply->user_id === auth()->id() ? 'text-right mr-1' : 'ml-1'); ?>">
                                    <?php echo e($reply->user->name); ?> <?php if($reply->is_admin): ?> <span class="text-[#0071c5] font-black uppercase text-[8px] ml-1">Staff</span> <?php endif; ?> · <?php echo e($reply->created_at->diffForHumans()); ?>

                                </p>
                                <div class="px-4 py-3 shadow-sm <?php echo e($reply->user_id === auth()->id() ? 'bg-[#0071c5] text-white rounded-tl-2xl rounded-bl-2xl rounded-br-2xl' : 'bg-white border border-gray-200 text-gray-800 rounded-tr-2xl rounded-br-2xl rounded-bl-2xl'); ?>">
                                    <p class="text-sm leading-relaxed"><?php echo e($reply->message); ?></p>
                                </div>
                                <?php if($reply->image): ?>
                                    <img src="<?php echo e(asset('storage/' . $reply->image)); ?>" alt="attachment" class="mt-2 max-w-[250px] rounded-xl <?php echo e($reply->user_id === auth()->id() ? 'ml-auto' : ''); ?> block border border-gray-100">
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <div class="bg-white border border-gray-100 shadow-sm p-6 rounded-xl">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-xs font-bold <?php echo e($ticket->status_badge); ?> px-2 py-0.5 rounded uppercase"><?php echo e($ticket->status_label); ?></span>
                    </div>
                    <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-xs text-gray-700 font-medium"><?php echo e($ticket->updated_at->diffForHumans()); ?></span>
                    </div>
                </div>
            </div>
