...
<div class="mb-5 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 dark:border-red-800 dark:bg-red-900/20">
    ...
    <div class="text-sm text-red-700 dark:text-red-400">
        <p>{{ $errors->first() }}</p>
        @if(session('show_ticket_link'))
            <a href="{{ route('guest.tickets.create') }}" class="mt-1 inline-block font-medium underline hover:text-red-800 dark:hover:text-red-300">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btn.addEventListener('click', function (e) {
            ...
            grecaptcha.ready(function () {
                grecaptcha.execute('{{ config('services.recaptcha.site_key') }}', { action: 'signin' })
                    .then(function (token) {
