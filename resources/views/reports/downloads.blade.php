...
<div class="p-4 mx-auto w-full md:p-6">
    ...
    <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        {{-- ✅ PDF Download button — current filter shoho --}}
        <a href="{{ route('reports.downloads.pdf', request()->query()) }}"
    ...
    <div class="rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-4 mb-5">
        <form method="GET" action="{{ route('reports.downloads') }}" id="filterForm">
            ...
            {{-- Custom Date Range --}}
            <div class="flex flex-wrap items-center gap-3 {{ $filter !== 'custom' ? 'hidden' : '' }}" id="customRangeBox">
                {{-- ✅ Hidden 'filter' input REMOVE kora holo - button er value already ache --}}
                <div>
                    <label class="text-xs text-gray-500 dark:text-gray-400 block mb-1">From</label>
                    <input type="date" name="date_from" value="{{ request('date_from', $startDate->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="date_to" value="{{ request('date_to', $endDate->format('Y-m-d')) }}"
                        class="h-10 rounded-lg border border-gray-300 bg-transparent px-3 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
            ...
            <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                {{-- ✅ Ei hidden 'filter' input-o REMOVE kora holo --}}
                @if($filter === 'custom')
    ...
    <div x-show="open" x-cloak
        ...
        <template x-if="selectedId">
            <div @click="clearSelection()"
                ...
                {{-- ✅ Filter thik rakhার jonno ekটা button-e-i submit, kিন্তু current filter-ke hidden field diye pathাও ALADA name diye --}}
                <input type="hidden" name="current_filter" value="{{ $filter }}">

                <a href="{{ route('reports.downloads', ['filter' => $filter]) }}"
                ...
                @if(request()->anyFilled(['search', 'model']))
                <a href="{{ route('reports.downloads', ['filter' => $filter]) }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
        <div class="rounded-xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-4">
                ...
                <h4 class="mt-1 font-bold text-gray-800 text-2xl dark:text-white/90">{{ $totalDownloads }}</h4>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-4">
                ...
                <h4 class="mt-1 font-bold text-gray-800 text-2xl dark:text-white/90">{{ $uniqueUsers }}</h4>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-4">
                ...
                <h4 class="mt-1 font-bold text-gray-800 text-2xl dark:text-white/90">{{ $uniqueAssets }}</h4>
            </div>
        </div>
    </div>
                    ...
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-2">
                                @if($log->user?->avatar)
                                <img src="{{ asset('storage/' . $log->user->avatar) }}" class="w-7 h-7 rounded-full object-cover">
                                @else
                                ...
                                @endif
                                <div>
                                    <p class="font-medium text-gray-800 dark:text-white/90">{{ $log->user->name ?? 'Unknown' }}</p>
                                    <p class="text-xs text-gray-400">{{ $log->user->email ?? '' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-gray-700 dark:text-gray-300">{{ $log->display_name }}</td>
                        <td class="py-3 px-4 font-semibold text-blue-600 dark:text-blue-400">{{ $log->count }}</td>
                        <td class="py-3 px-4 text-gray-400 text-xs">{{ $log->ip_address ?? '—' }}</td>
                        <td class="py-3 px-4 text-gray-400 text-xs">{{ $log->updated_at->format('d M Y, h:i A') }}</td>
                    </tr>
                    @empty
                    <tr>
